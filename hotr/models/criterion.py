...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, num_actions=None, HOI_losses=None, HOI_matcher=None, args=None):
        ...
        """
        super().__init__()
        self.task = args.task #　for ASOD
        self.num_classes = num_classes
        self.matcher = matcher
        self.weight_dict = weight_dict
        if self.task == 'ASOD':
            self.weight_dict['loss_soidx'] = args.hoi_soidx_loss_coef  # Second Objectの損失係数を追加
        self.losses = losses
    ...
    def loss_pair_labels(self, outputs, targets, hoi_indices, num_boxes, log=False):
        ...
        if self.task == 'ASOD':
            target_soidx_classes = torch.full(src_soidx.shape[:2], -1, dtype=torch.int64, device=src_soidx.device)
        ...
        # S Pointer loss
        if self.task == 'ASOD':            
            target_classes_so = torch.cat([t["so_labels"][J] for t, (_, J) in zip(targets, hoi_indices)])
            target_soidx_classes[idx] = target_classes_so
        ...
        if self.task == 'ASOD':            
            # 損失の計算（
            loss_soidx = F.cross_entropy(src_soidx.transpose(1, 2), target_soidx_classes, ignore_index=-1)
            
            losses['loss_soidx'] = loss_soidx  # 既存のlossesに追加する

        return losses

    
    # >>> HOI Losses 1-2 : S(Second Object) Pointer 
    # hoi_indices内のインデックスJが空である場合に、損失を計算しないようにするバージョン
    def loss_soidx(self, outputs, targets, hoi_indices, num_boxes, log=False):
        assert 'pred_soidx' in outputs
        src_soidx = outputs['pred_soidx']
        idx = self._get_src_permutation_idx(hoi_indices)

        # ターゲットのSecond Objectのラベルを取得
        target_soidx_classes = torch.full(src_soidx.shape[:2], -1, dtype=torch.int64, device=src_soidx.device)
