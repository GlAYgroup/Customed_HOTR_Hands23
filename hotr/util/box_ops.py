...
def rescale_triplet(out_pairs, size):
    img_h, img_w = size
    h_bbox = out_pairs[:, :4]
    o_bbox = out_pairs[:, 4:8]
    so_bbox = out_pairs[:, 8:12]
    ...
    sobj_mask = (so_bbox[:, 0] != -1)
    if sobj_mask.sum() != 0:
        so = box_cxcywh_to_xyxy(so_bbox)
        so = so * torch.tensor([img_w, img_h, img_w, img_h], dtype=torch.float32).to(so_bbox.get_device())
        so_bbox[sobj_mask] = so[sobj_mask]
    so = so_bbox
