...
class APRole(object):
    def __init__(self, act_name, scenario_flag=True, iou_threshold=0.5):
        ...
        # HOIペアを保存するためのデータ構造
        self.hoi_pairs = {act: [] for act in act_name}

    def add_data(self, h_box, o_box, score, i_box, i_act, p_box, p_act):
        # hbox, obox, score, gt_h_box, gt_h_act, gt_p_box, gt_p_act
        ...
        # print("size of h_box", h_box.shape)
        ...
        # print("i_act", i_act)
        # print("valid_i_inds", valid_i_inds)
        # print("overlaps", overlaps)
        # print("assigned_input", assigned_input)
        # print("v_inds", v_inds)
        # print("v_inds.shape", v_inds.shape)
        ...
        # print("size of h_box", h_box.shape)
        # print("size of score", score.shape)
        # print("size of p_act", p_act.shape)
        ...
        if n_o == 0:
            ...
            s_obj_argmax = np.argmax(score.reshape(-1, n_o), axis=1).reshape(-1, n_h) # (#act, n_h) それぞれのpredictされたh_boxに対する一番pair_scoreの高いo_boxのindex
            s_obj_max = np.max(score.reshape(-1, n_o), axis=1).reshape(-1, n_h) # (#act, n_h)　それぞれのpredictされたh_boxに対する一番値の高いpair_score

            h_overlaps = compute_overlap(h_box, p_box[:, :4]) # (n_h, n_p)

            # print("len(self.act_name)" ,len(self.act_name))
            # print("s_obj_argmax.shape", s_obj_argmax.shape)
            # print("s_obj_max.shape", s_obj_max.shape)


            for label in range(len(self.act_name)):
                h_inds = np.argsort(s_obj_max[label])[::-1] # (n_h, ) #あるactionの中で予測されたh_boxをpair_scoreの高い順に並べる
                ...
                # print("h_max_overlap", h_max_overlap)
                ...
                # print("p_inds", p_inds)
                # print("h_overlaps", h_overlaps)
                # print("h_max_temp", h_max_temp)
                # print("h_over_thresh", h_over_thresh)
                # print("np.expand_dims(p_inds, axis=0)", np.expand_dims(p_inds, axis=0))
